/**
 * MISE EN FORME ET VERIFICATIONS DYNAMIQUES DU FORMULAIRE DE COMMANDE. (jquery et ajax utilisés)
 */

        	// fonction jQuery permettant de rendre le formulaire et les VERIFICATIONS dynamiques.
	        $(document).ready(function(){
	        	
	        		//déclaration des variables récupérant les champs du formulaire de PASSAGE DE COMMANDE.
	            	var $nomexp = $('#nom_expediteur'),
		            	...
		            	*/
	            		...
	            		$form.submit(function(){
	            			valid = true;
	            			if(validExp && validDest && verifierVal($val) && verifierPoids($poids) && verifierDim($longueur) && verifierDim($largeur) && verifierDim($hauteur) && verifierDate($datepicker)){
	            				valid = true;
	            			} else {
	            				valid = false; //empêche le submit
	            			}
	            			return valid;
	            	    });
	            		
	            		/*Vérification avant envoi du formulaire du carnet*/
	            		$formCarnet.submit(function(){
	            			valid = true;
	            			/*on vérifie tous les champs destinataire*/
	            	       	if((verifier($nomCarnet) && 
		            	       	verifier($prenomCarnet) &&
		            	       	verifierCodeDest($codedest, $paysdest) &&
		            	       	verifier($locdest) &&
		            	       	verifierRue($ruedest) &&
		            	       	verifierNum($numdest) &&
	            	 	...
	            	 	$codedest.autocomplete({
	            	 		 source : function(requete, reponse){ // les deux arguments représentent les données nécessaires au plugin
	            	 			$.ajax({
	            	 		            ...
	            	 		            success : function(donnee){
	            	 		                reponse($.map(donnee.postalCodes, function(objet){
	            	 		                	$codedest.css({ // on rend le champ vert
	            	            	    	        borderColor : 'green'
	            	            	    	    });
	        			...
	        			var nameStartDest = $locdest.val();
	            	 	...
	            	 	//a chaque changement de la valeur estimée du colis
	            	 	$val.keyup(function(){
	            	       	verifierVal($(this));
	            	    });
	            	 	//a chaque changement du poids
	            	 	$poids.keyup(function(){
	            	       	verifierPoids($(this));
	            	    });
	            	 	//a chaque changement de la longueur, dimension pour le colis
	            	 	$longueur.keyup(function(){
	            	       	verifierDim($(this));
	            	    });
	            	 	//a chaque changement de la largeur
	            	 	$largeur.keyup(function(){
	            	       	verifierDim($(this));
	            	    });
	            	 	//a chaque changement de la hauteur
	            	 	$hauteur.keyup(function(){
	            	       	verifierDim($(this));
	            	    });


	            	 	//a chaque changement de prenom et nom carnet
	            	 	$nomCarnet.keyup(function(){
	            	       	verifier($(this));
	            	    });
	            	 	$prenomCarnet.keyup(function(){
	            	       	verifier($(this));
	            	    });
						...
						*/
	            	    ...
	            	    /*
							Fonction de vérification de formulaire pour la valeur du colis
						*/
	            	    function verifierVal(champ){
	            	        if(champ.val() == ""){ // si le champ est vide
	            	    	    champ.css({ // on rend le champ rouge
	            	    	        borderColor : 'red'
	            	    	    });
	            	    	    champ.next(".error").fadeIn().text("Veuillez entrer la valeur de votre colis");
	            				$(".erreur").hide();
	            				return false;
	            	        } //expr reguliere pour une nombre décimal
	            	        else if (!champ.val().match(/^((0.[1-9][0-9]?)|(0.[0-9]?[1-9])|([1-9][0-9]*(.[0-9][0-9]?)?))$/i)) { 
	            	        	champ.css({ // on rend le champ rouge
	            	    	        borderColor : 'red'
	            	    	    });
	            	    	    champ.next(".error").fadeIn().html(champ.val() + " n'est pas une valeur valide.</br>Veuillez entrer un nombre décimal</br>positif non-nul avec maximum</br>2 chiffres après la virgule.");
	            				$(".erreur").hide();
	            				return false;
	            	        }
	            	        else{
	            	    ...
	            	    /*
							Fonction de vérification de formulaire pour le poids
						*/
	            	    function verifierPoids(champ){
	            	        if(champ.val() == ""){ // si le champ est vide
	            	    	    champ.css({ // on rend le champ rouge
	            	    	        borderColor : 'red'
	            	    	    });
	            	    	    champ.next(".error").fadeIn().text("Veuillez entrer le poids de votre colis");
	            				$(".erreur").hide();
	            				return false;
	            	        } //expr reguliere pour un nombre décimal
	            	        else if (!champ.val().match(/^((0.[0-9]?[1-9][0-9]?)|(0.[1-9][0-9]?[0-9]?)|(0.[0-9]?[0-9]?[1-9])|([1-9][0-9]*(.[0-9][0-9]?[0-9]?)?))$/i)) { 
	            	        	champ.css({ // on rend le champ rouge
	            	    	        borderColor : 'red'
	            	    	    });
	            	    	    champ.next(".error").fadeIn().html(champ.val() + " n'est pas une valeur valide.</br>Veuillez entrer un nombre décimal</br>positif non-nul avec maximum</br>3 chiffres après la virgule.");
	            				$(".erreur").hide();
	            				return false;
	            	        }
	            	        else if (champ.val() > 100 ) { 
	            	        	champ.css({ // on rend le champ rouge
	            	    	        borderColor : 'red'
	            	    	    });
	            	    	    champ.next(".error").fadeIn().text("Le poids du colis ne peut pas excéder 100 kg");
	            				$(".erreur").hide();
	            				return false;
	            	        }
	            	        else{
						...
						*/
	            	    function verifierDate(champ){
	            	    	if(champ.val() == ""){ // si le champ est vide
	            	    	    ...
	            	    	    champ.next(".error").fadeIn().text("Veuillez indiquer la date de livraison souhaitée.");
	            	            $(".erreur").hide();
	            	            return false;
	            	    ...
	            	    /*
							Fonction de vérification de formulaire pour le poids
						*/
	            	    function verifierDim(champ){
	            	        if(champ.val() == ""){ // si le champ est vide
	            	    	    champ.css({ // on rend le champ rouge
	            	    	        borderColor : 'red'
	            	    	    });
	            	    	    $largeur.next(".error").fadeIn().text("Veuillez entrer les dimensions de votre colis");
	            				$(".erreur").hide();
	            				return false;
	            	        } //expr reguliere pour un nombre décimal
	            	        else if (!champ.val().match(/^[1-9][0-9]*$/i)) { 
	            	        	champ.css({ // on rend le champ rouge
	            	    	        borderColor : 'red'
	            	    	    });
	            	        	$largeur.next(".error").fadeIn().text("Veuillez entrer des dimensions valides (pas de nombre à virgule).");
	            				$(".erreur").hide();
	            				return false;
	            	        }
	            	        else if (champ.val() > 150 ) { 
	            	        	champ.css({ // on rend le champ rouge
	            	    	        borderColor : 'red'
	            	    	    });
	            	        	$largeur.next(".error").fadeIn().text("Les dimensions du colis ne peuvent pas excéder 150 cm");
	            				$(".erreur").hide();
	            				return false;
	            	        }
	            	        else{
	            	        	...
	            	        	$largeur.next(".error").fadeOut('slow');
	            	            $(".erreur").hide();
	            	            return true;
	            	        }
	            	    }
            ...
            jQuery(document).ready(function(){
                ...
                jQuery('select[name=CarnetDest]').click(function(){
                	if(document.getElementById("CarnetDest").value == "aucune"){
                        ...
                        $("input[name=adresse_loc_destinataire]:text").show("slow", function(){});
                        $("input[name=adresse_code_destinataire]:text").show("slow", function(){});
                		$("div#noCarnetDest").show("slow", function(){});
                		$("div#precDest").show("slow", function(){});
                		$("div#suivDest").show("slow", function(){});
                		var divId = jQuery(this).val();
                ...
                /* Click barre renseignement adresse On affiche les informations sur les adresses et cache celles sur le colis. */
