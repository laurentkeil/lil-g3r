/**
 * MISE EN FORME ET VERIFICATIONS DYNAMIQUES DU FORMULAIRE D EDITION DE PROFIL. (jquery et ajax utilisés)
 */
	        ...
	        $(document).ready(function(){
            	...
            	var $motdepasse = $('#motdepasse'),
	                ...
	                $form.submit(function(){
		            	if($motdepasse.val().length != 0 && !result){ //mdp robuste
		            		$motdepasse.next(".error").fadeIn('slow').text("Le mot de passe doit être au moins de sécurité moyenne.");  	                		
		            		$('.mdpbar').hide();
		            		return false;
		            	}
		            	if ( result && conf && verifier($nom) && verifier($prenom) && verifier($adresse_pays) && verifierCode($adresse_code, $adresse_pays) && verifier($adresse_loc) 
	                ...
	                $motdepasse.keyup(function(){
	        	        ...
	        	        $(".erreur").hide();
	        	       	if(mdp.length != 0){
	        	       		...
	        	       		if( ( (mdp.match(lettre_min)) || (mdp.match(lettre_maj)) || (mdp.match(nombre)) ) && (mdp.length < 5)){
	        	       		...
	        	       		//password moyen	(min et maj OU min et nombre OU maj et nombre ET plus de 5 caractères et moins de 8) / accepté
	        	       		if( ( ((mdp.match(lettre_min)) && (mdp.match(lettre_maj))) || ((mdp.match(nombre)) && (mdp.match(lettre_maj))) || ((mdp.match(lettre_min)) && (mdp.match(nombre)) ) ) && (mdp.length >= 5) ){
					...
					*/
            	    ...
            	    function verifier(champ){
            	        ...
            	        if (!champ.val().match(/^[A-Za-zàáâãäåçèéêëìíîïðòóôõöùúûüýÿ\-\ ]*$/i) ) {
            	    ...
            	    /*
            	     * Verification pour la confirmation de mdp
            	     */
	        	    function verifierConf(champ){
		        	   if (champ.val() != $motdepasse.val()){
	                		champ.css({ // on rend le champ rouge
	        	    	        borderColor : 'red'
	        	    	    });
					...
					*/
            	    function verifierRue(champ){
            	        if (!champ.val().match(/^[A-Za-z0-9àáâãäåçèéêëìíîïðòóôõöùúûüýÿ\-\'\ ]*$/i) ) {
            	        	champ.css({ // on rend le champ rouge
            	    	        borderColor : 'red'
            	    	    });
					...
					*/
            	    function verifierNum(champ){
            	        if ((!champ.val().match(/^[0-9]*$/i)) || (champ.val() > 3000) ) { //si le champ ne contient pas que des chiffres.
            	        	champ.css({ // on rend le champ rouge
            	    	        borderColor : 'red'
            	    	    });
					...
					*/
            	    function verifierDate(champ){
            	    	...
            	    	if ((!champ.val().match(/^(((0[1-9])|(1\d)|(2\d)|(3[0-1]))\/((0[1-9])|(1[0-2]))\/(\d{4}))$/i) ) ) {
            	        	champ.css({ // on rend le champ rouge 
            	    	        borderColor : 'red'
            	    	    });alert("ok");
